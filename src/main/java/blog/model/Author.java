...
@Entity
@Table(name = "authors")
@NamedQuery(name = "Author.findAllWithPosts",query = "select distinct a from Author a left join  a.posts p")
@NamedQuery(name = "Author.findByName",query = "select distinct a from Author a left join fetch a.posts where a.name = :name")
